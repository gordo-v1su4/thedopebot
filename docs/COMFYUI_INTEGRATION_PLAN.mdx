...
ComfyUI also mounts duplicated `/api/*` route prefixes for easier delegation in frontends/reverse proxies.
...
- `index.js`: façade used by route handlers and AI tools.
...
- ComfyUI core (`Comfy-Org/ComfyUI`)
- TS SDKs:
  - `comfyui-node` (active, TypeScript)
  - `@saintno/comfyui-sdk` (active, TypeScript)
- Conversion / adapter ecosystem:
  - `SethRobinson/comfyui-workflow-to-api-converter-endpoint`
  - `Ckrest/comfyui-conduit`
