...
export async function getOpenRouterModels(options = {}) {
  ...
  const enabled = provider === 'custom' && baseUrl.includes('openrouter.ai');
  ...
  if (!apiKey) {
    return {
      ...
      recommendedModel: null,
      fetchedAt: new Date().toISOString(),
      error: 'CUSTOM_API_KEY is not configured for OpenRouter.',
    };
  }
...
/**
 * Get effective LLM provider/model used by chat.
 * @returns {Promise<{provider: string, model: string, label: string}>}
 */
export async function getChatModelInfo() {
  await requireAuth();
  const { getConfiguredModel } = await import('../ai/model.js');
  const { provider, modelName } = getConfiguredModel();
  return {
    provider,
    model: modelName,
