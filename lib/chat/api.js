...
export async function POST(request) {
  ...
  const { messages, chatId: rawChatId, trigger, llmProvider, llmModel } = body;
  const allowedProviders = new Set(['anthropic', 'openai', 'google', 'custom']);
  const sanitizedProvider =
    typeof llmProvider === 'string' && allowedProviders.has(llmProvider) ? llmProvider : undefined;
  const sanitizedModel =
    typeof llmModel === 'string' &&
    llmModel.trim().length > 0 &&
    llmModel.trim().length <= 200 &&
    /^[a-zA-Z0-9._:/-]+$/.test(llmModel.trim())
      ? llmModel.trim()
      : undefined;
  ...
  const effectiveProvider = sanitizedProvider || process.env.LLM_PROVIDER || 'anthropic';
  const effectiveModel = sanitizedModel || process.env.LLM_MODEL || '';
  console.info(
    `[chat/api] provider=${effectiveProvider} model=${effectiveModel || '(default)'} thread=${threadId}`
  );
  ...
  const stream = createUIMessageStream({
    ...
    execute: async ({ writer }) => {
      ...
      const chunks = chatStream(threadId, userText, attachments, {
        ...
        skipUserPersist,
        llmProvider: sanitizedProvider,
        llmModel: sanitizedModel,
      });
