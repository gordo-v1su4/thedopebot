...
export function getConfiguredModel(options = {}) {
  const provider = options.provider || process.env.LLM_PROVIDER || 'anthropic';
  const modelName =
    options.model || process.env.LLM_MODEL || DEFAULT_MODELS[provider] || DEFAULT_MODELS.anthropic;
  const maxTokens = options.maxTokens || Number(process.env.LLM_MAX_TOKENS) || 4096;
  const baseURL = process.env.OPENAI_BASE_URL;
  return { provider, modelName, maxTokens, baseURL };
}
 ...
 * @param {string} [options.provider] - Provider override for this call
 * @param {string} [options.model] - Model override for this call
 * @returns {import('@langchain/core/language_models/chat_models').BaseChatModel}
 */
export async function createModel(options = {}) {
  const { provider, modelName, maxTokens, baseURL } = getConfiguredModel(options);
